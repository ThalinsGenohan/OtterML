...
public:
	...
	[[nodiscard]] const std::array<T, Tx * Ty>& GetData() const;
	[[nodiscard]] T GetValue(u32 row, u32 column) const;

	void SetData(const std::array<T, Tx * Ty>& data);
...
private:
	...
	std::array<T, Tx * Ty> _data;

	// Function to get cofactor of A[p][q] in temp[][]. n is
	// current dimension of A[][]
	static void GetCofactor(const std::array<T, Tx * Ty>& A, std::array<T, Tx * Ty>& temp, i32 p, i32 q, i32 n)
	{
		i32 i = 0;
		i32 j = 0;
	...
	static T GetDeterminant(std::array<T, Tx * Ty> A, i32 n)
	{
		...
		std::array<T, Tx * Ty> temp; // To store cofactors
		...
		for (i32 f = 0; f < n; f++)
		{
			...
			D += sign * A[f] * GetDeterminant(temp, n - 1);
	...
	std::array<T, Tx * Ty> GetAdjoint() const
	{
		std::array<T, Tx * Ty> adj;
		if constexpr (Tx == 1)
		{
			adj[0] = 1;
			return adj;
		...
		for (i32 i = 0; i < Ty; i++)
		{
			for (i32 j = 0; j < Tx; j++) {
				...
				adj[j * Tx + i] = (sign) * (GetDeterminant(temp, Tx - 1));
			}
		}
		return adj;
	}
...
Matrix<T, Tx, Ty>::Matrix(bool identity) : Matrix(static_cast<T>(0))
{
	...
	for (u32 i = 0; i < Tx; i++)
	{
		this->_data[i * Tx + i] = static_cast<T>(1);
	}
}
...
Matrix<T, Tx, Ty>::Matrix(const T scalar)
{
	for (u32 y = 0; y < Ty; y++)
	{
		for (u32 x = 0; x < Tx; x++)
		{
			this->_data[y * Tx + x] = scalar;
		}
	}
}

template <typename T, unsigned Tx, unsigned Ty>
const std::array<T, Tx * Ty>& Matrix<T, Tx, Ty>::GetData() const
{
	return this->_data;
}
template <typename T, unsigned Tx, unsigned Ty>
...
{
	return this->_data[row * Tx + column];
}

template <typename T, unsigned Tx, unsigned Ty>
void Matrix<T, Tx, Ty>::SetData(const std::array<T, Tx * Ty>& data)
{
	this->_data = data;
}
...
Matrix<T, Tx, Ty> Matrix<T, Tx, Ty>::GetInverse() const
{
	...
	// Find adjoint
	std::array<T, Tx * Ty> adj = this->GetAdjoint();
	...
	for (i32 i = 0; i < Ty; i++)
	{
		for (i32 j = 0; j < Tx; j++)
		{
			inverse.SetValue(i, j, adj[i * Tx + j] / f32(det));
		}
	}
